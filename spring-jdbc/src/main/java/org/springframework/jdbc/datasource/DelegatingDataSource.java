...
/**
 * DelegatingDataSource的作用是，它自身持有一个其他的DataSource实例作为目标对象，当getConnection()等方法被调用的时候，
 * DelegatingDataSource会将调用转发给持有的这个DataSource实例
 * DelegatingDataSource本身在转发这些调用之前没有做任何事情，这是没有任何意义的
 * 要发挥DelegatingDataSource的作用，我们在实现DelegatingDataSource子类的时候，需要覆写相应的方法，在转发方法调用之前添加相应的自定义逻辑
 */
