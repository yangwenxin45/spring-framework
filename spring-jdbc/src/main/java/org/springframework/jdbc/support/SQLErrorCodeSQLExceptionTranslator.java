...
/**
 * Implementation of {@link org.springframework.jdbc.support.SQLExceptionTranslator} that analyzes vendor-specific error codes.
 ...
 * <li>Fallback to a fallback translator. {@link org.springframework.jdbc.support.SQLStateSQLExceptionTranslator} is the
 ...
 * use {@link org.springframework.jdbc.support.SQLExceptionSubclassTranslator} by default, which in turns falls back
...
/**
 * SQLErrorCodeSQLExceptionTranslator会基于SQLException所返回的ErrorCode进行异常转译
 * 通常情况下，根据各数据库提供商所提供的ErrorCode进行分析要比基于SQLState的方式准确得多
 * 默认情况下，JdbcTemplate采用SQLErrorCodeSQLExceptionTranslator进行SQLException的转移工作，
 * 只有当通过ErrorCode方式无法提供足够信息的时候，才会转而求助于SQLStateSQLExceptionTranslator
 */
public class SQLErrorCodeSQLExceptionTranslator extends AbstractFallbackSQLExceptionTranslator {
    ...
    /**
     * 转译流程：
     * 1. 程序流程首先会检查该自定义异常转译方法是否能够对当前传入的SQLException进行转译，如果可以，则直接返回转译后的DataAccessException类型；
     * 如果返回null，则表示当前方法无法对传入的SQLException进行转译，程序流程将进入下一步，寻求其他方式进行转译
     * 在SQLErrorCodeSQLExceptionTranslator中，该方法直接返回null，所以，程序流程进入下一步
     * 2. 如果应用程序运行于Java 6之上，那么，SQLErrorCodeSQLExceptionTranslator会尝试让SQLExceptionSubclassTranslator进行异常的转译。
     * 对于Java 6之前版本的应用来说，这一步基本可以忽略，了解即可
     * 3. 使用org.springframework.jdbc.support.SQLErrorCodesFactory所加载的SQLErrorCodes进行异常转译，其中，SQLErrorCodesFactory加载SQLErrorCodes的流程为：
     * *加载位于Spring发布jar包中org/springframework/jdbc/support/sql-error-codes.xml路径下的记载了各个数据库提供商ErrorCode的配置文件，提取相应的SQLErrorCodes
     * *如果发现当前应用的Classpath的根路径下存在名称为sql-error-codes.xml的配置文件，则加载该文件内容，并覆盖默认的ErrorCode定义
     * 4. 如果基于ErrorCode的异常转译还是搞不定的话，SQLErrorCodeSQLExceptionTranslator将求助于SQLStateSQLExceptionTranslator，
     * 最后使用基于SQLState的方式进行SQLException到Spring数据访问异常体系的转移工作
     * <p>
     * 自定义异常转译的两种情况：
     * 1. 扩展SQLErrorCodesSQLExceptionTranslator
     * 2. 提供sql-error-codes.xml自定义配置
     *
     * @author yangwenxin
     * @date 2023-06-27 14:37
     */
    @Override
    @Nullable
