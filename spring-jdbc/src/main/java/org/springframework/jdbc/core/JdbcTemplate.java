 ...
 * them a clearly defined contract. The {@link org.springframework.jdbc.core.PreparedStatementCreator} callback
 * interface creates a prepared statement given a Connection, providing SQL and
 * any necessary parameters. The {@link org.springframework.jdbc.core.ResultSetExtractor} interface extracts
 * values from a ResultSet. See also {@link org.springframework.jdbc.core.PreparedStatementSetter} and
 ...
 * @see org.springframework.jdbc.core.PreparedStatementCallback
...
/**
 * JdbcTemplate是整个Spring数据抽象层提供的所有JDBC API最佳实践的基础
 * 主要关注两件事情：
 * 1. 封装所有基于JDBC的数据访问代码，以统一的格式和规范来使用JDBC API
 * 2. 对SQLException所提供的异常信息在框架内进行统一转译，将基于JDBC的数据访问异常纳入Spring自身的异常层次体系中,
 * 统一了数据接口的定义，简化了客户端代码对数据访问异常的处理
 */
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
     ...
     */
    // 当调用rs.next时，ResultSet会一次性从服务器上取得多少行数据回来
        ...
        return execute(psc, new PreparedStatementCallback<T>() {
            ...
            public T doInPreparedStatement(PreparedStatement ps) throws SQLException {
                ...
                try {
                    ...
                    rs = ps.executeQuery();
                    // 负责将结果进行封装并转换至POJO，res当前代表的类为RowMapperResultSetExtractor
                    return rse.extractData(rs);
                } finally {
                    JdbcUtils.closeResultSet(rs);
    ...
    /**
     * 该模板方法的主要好处是可以通过List指定存储过程的参数列表，
     * 之后，JdbcTemplate会根据指定的参数列表所提供的参数信息为我们组装调用结果，并以Map形式返回
     * declaredParameters参数列表中的元素，需要是org.springframework.jdbc.core.SqlParameter类型或者相关子类，
     * 声明顺序和参数类型要与实际存储过程定义的参数顺序和类型相同
     *
     * @author yangwenxin
     * @date 2023-06-27 16:22
     */
    @Override
