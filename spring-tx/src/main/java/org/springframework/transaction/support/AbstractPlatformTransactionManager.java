...
/**
 * AbstractPlatformTransactionManager作为DataSourceTransactionManager的父类，以模板方法的形式封装了固定的事务处理逻辑，
 * 而只将与事务资源相关的操作以protected或者abstract方法的形式留给DataSourceTransactionManager来实现
 * <p>
 * 作为模板方法父类，AbstractPlatformTransactionManager替各子类实现了一下固定的事务内部处理逻辑：
 * 1. 判定是否存在当前事务，然后根据判断结果执行不同的处理逻辑
 * 2. 结合是否存在当前事务的情况，根据TransactionDefinition中指定的传播行为的不同语义执行后继逻辑
 * 3. 根据情况挂起或者恢复事务
 * 4. 提交事务之前检查readOnly字段是否被设置，如果是的话，以事务的回滚代替事务的提交
...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
    ...
    public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
        ...
        // 判断当前线程是否存在事务，判断依据为当前线程记录的连接不为空且连接(connectionHolder)中的transactionActive属性不为空
        ...
        try {
            ...
            try {
                ...
                if (!beforeCompletionInvoked) {
                    // 添加的TransactionSynchronization中的对应方法调用
                    triggerBeforeCompletion(status);
                }
                // 提交过程中出现异常则回滚
                doRollbackOnCommitException(status, ex);
